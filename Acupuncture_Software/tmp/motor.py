...
#     if math.fabs(v) < 1e-6:     # 若速度等于零, dir = 0, 表示立即停止
#         return 0
#     elif v > 0:                 # 若速度大于零, dir = -1, 实现前进
#         return -1
#     elif v < 0:                 # 若速度小于零, dir = 1, 实现后退
#         return 1
...
#     if math.fabs(v) < 1e-6:     # 若速度等于零, dir = 0, 表示立即停止
#         return 0
#     elif v > 0:                 # 若速度大于零, dir = 1, 表示顺时针旋转
#         return 1
#     elif v < 0:                 # 若速度小于零, dir = -1, 表示逆时针旋转
#         return -1
...
# add
def posConvert1(posRead1):
    # 电机1相对运动的脉冲数posRead1
    # posReal1 单位为 mm
    posReal1 = posRead1 / 256 / 2002
    return posReal1

def posConvert2(posRead2):
    # 电机2相对运动的脉冲数posRead2
    # posReal2 单位为 r
    posReal2 = posRead2 / 256 / 4009
    return posReal2
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if Mode == 0:                                                                           # 停止运行
            motor1.write([0x01, 0x06, 0x00, 0xC8, 0x00, 0x00, 0x08, 0x34])
            motor2.write([0x02, 0x06, 0x00, 0xC8, 0x00, 0x00, 0x08, 0x07])
            print("正在结束运行")
            break
        elif Mode == 1:                                                                         # 设置绝对位置
            ...
            if ver == '1':
                ...
                if equipNum == '1':
                    motor1.write([0x01, 0x10, 0x00, 0xD2, 0x00, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00, 0x7F, 0x2A])
                    time.sleep(0.01)
                    motor1.write([0x01, 0x06, 0x00, 0xDC, 0x00, 0x01, 0x89, 0xF0])
                    time.sleep(0.01)
                elif equipNum == '2':  # 站号为0x02?
                    motor2.write([0x01, 0x10, 0x00, 0xD2, 0x00, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00, 0x7F, 0x2A])
                    time.sleep(0.01)
                else:
                    print("非法输入, 重新运行")
                    continue
            ...
            # 运行到绝对位置为10000脉冲数
            motor1.write([0x01, 0x10, 0x00, 0xD0, 0x00, 0x02, 0x04, 0x27, 0x10, 0x00, 0x00, 0xF5, 0x82])
            ...
            time.sleep(0.005)  # 延时5ms
            lCur1 = readLocation(motor1)  # 读取当前绝对位置
            print('lCur1Start:', lCur1)
            lCur2 = readLocation(motor2)  # 读取当前绝对位置
            print('lCur2Start:', lCur2)
            ...
            time.sleep(0.005)                                                                   # 延时5ms
            lCur1 = readLocation(motor1)                                                        # 读取当前绝对位置, 单位1/100000圈??
            print('lCur1End:', lCur1)
            lCur2 = readLocation(motor2)
            print('lCur2End:', lCur2)

            dirCur1 = Dir1  # 更新当前方向状态
            dirCur2 = Dir2

        elif Mode == 5:
            # b = readLocation(motor2)  # 将之前接受的信息读出, 清空接收缓冲区
            # motor2.write([0x02, 0x03, 0x00, 0x04, 0x00, 0x02, 0x85, 0xF9])
            # time.sleep(0.005)  # 延时5ms
            # lCur2 = readLocation(motor2)  # 读取当前绝对位置
            # print("lCur2 pre:", lCur2)

            # # 运行到绝对位置为5000脉冲数
            # motor2.write([0x02, 0x10, 0x00, 0xD0, 0x00, 0x02, 0x04, 0x13, 0x88, 0x00, 0x00, 0x75, 0x19])

            # # 相对当前位置运行5000脉冲数
            # motor2.write([0x02, 0x10, 0x00, 0xDE, 0x00, 0x02, 0x04, 0x13, 0x88, 0x00, 0x00, 0xF4, 0x95])

            # 运行到绝对位置为10000脉冲数
            motor2.write([0x02, 0x10, 0x00, 0xD0, 0x00, 0x02, 0x04, 0x27, 0x10, 0x00, 0x00, 0xFA, 0xC6])

        elif Mode == 6:
            a = readLocation(motor1)        # 将之前接受的信息读出, 清空接收缓冲区
            time.sleep(0.005)  
            spl = motor1.write([0x01, 0x03, 0x00, 0x9B, 0x00, 0x01, 0xF5, 0xE5])
            time.sleep(0.01)
            # spl = readLocation(motor1)
            print('spl:', spl)  
            # motor1.write([0x01, 0x06, 0x00, 0xDC, 0x00, 0x01, 0x89, 0xF0])
