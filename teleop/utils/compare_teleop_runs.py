...
def build_report(*, mpc: RunData, ik: RunData, out_html: Path, title: str) -> None:
    ...
    # Optional: scale MPC time axis (do NOT trim samples) so its total duration ~= scale * IK duration.
    # This makes plots comparable when MPC run has more idle/wait time but the spatial samples are still meaningful.
    mpc_time_scale_applied = 1.0
    try:
        ik_dur = float(t_ik[-1]) if t_ik.size > 0 else 0.0
        mpc_dur = float(t_mpc_raw[-1]) if t_mpc_raw.size > 0 else 0.0
        target_mpc_dur = float(max(0.0, MPC_TIME_SCALE * ik_dur))
        if ik_dur > 1e-9 and mpc_dur > 1e-9 and target_mpc_dur > 1e-9:
            # Linear scaling factor applied to MPC time axis
            mpc_time_scale_applied = target_mpc_dur / mpc_dur
    except Exception:
        mpc_time_scale_applied = 1.0

    t_mpc = t_mpc_raw * float(mpc_time_scale_applied)

    # 不做长度截断：mpc/ik 采样点数允许不同，只要各自从 t=0 开始。

    # record the applied scaling for report/debug
    try:
        mpc.meta = dict(mpc.meta)
        mpc.meta["report_mpc_time_scale_applied"] = float(mpc_time_scale_applied)
        mpc.meta["report_mpc_time_scale_target"] = float(MPC_TIME_SCALE)
    except Exception:
        pass
    ...
    try:
        ...
        def _add_run(run: RunData, *, color_ref: str, color_ee: str, dash_ref: str = "dash") -> None:
            ...
            # y
            fig_s.add_trace(go.Scatter(x=s_ref, y=run.xr_p[:, 1], name=f"{run.name} XR", line=dict(color=color_ref, dash=dash_ref), showlegend=False), row=1, col=2)
            fig_s.add_trace(go.Scatter(x=s_ee, y=run.ee_p[:, 1], name=f"{run.name} EE", line=dict(color=color_ee), showlegend=False), row=1, col=2)
            # z
            fig_s.add_trace(go.Scatter(x=s_ref, y=run.xr_p[:, 2], name=f"{run.name} XR", line=dict(color=color_ref, dash=dash_ref), showlegend=False), row=1, col=3)
            fig_s.add_trace(go.Scatter(x=s_ee, y=run.ee_p[:, 2], name=f"{run.name} EE", line=dict(color=color_ee), showlegend=False), row=1, col=3)
    ...
    if fig_s is not None:
        html_parts.append("<h2>弧长参数对齐：x(s)/y(s)/z(s)（横向排版）</h2>")
        ...
        html_parts.append("<p>未生成弧长参数对齐图（数据不足）。</p>")
