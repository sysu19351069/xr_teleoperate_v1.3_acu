...
if __name__ == '__main__':
    ...
    parser.add_argument('--compare-out-dir', type=str, default='./teleop_compare_logs', help='对比数据输出目录（npz）。')
    ...
    try:
        ...
        while not STOP:
                        ...
                        if (args.mpc_stream_on_trigger or args.mpc_stream_toggle) and (not args.mpc_test_input):
                            if args.xr_mode == 'hand':
                                ...
                                if mpc_ref_enabled and (teleop_ref is not None) and args.use_mpc_control:
                                    ...
                                    ik_sol = teleop_mpc_api.ctl._ik.solve(target_pose) if target_pose is not None else None
            ...
            try:
                # Only control in main loop when ACU PF thread is not responsible.
                if args.arm == 'ACU' and teleop_mpc_api is not None and not args.use_mpc_control:
                    arm_ctrl.ctrl_dual_arm(sol_q, sol_tauff)
            except Exception as e:
                pass
